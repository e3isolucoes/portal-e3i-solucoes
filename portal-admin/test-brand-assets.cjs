...
assert.match(snippet, /E3I_ADMIN_BRAND_ASSETS_PATCH_V1/);
...
assert.equal((patched.match(/E3I_ADMIN_BRAND_ASSETS_PATCH_V1/g) || []).length, 1);
...
assert.equal((patched.match(/E3I_ADMIN_BRAND_ASSETS_PATCH_V1/g) || []).length, 1, 'patch must be idempotent');
