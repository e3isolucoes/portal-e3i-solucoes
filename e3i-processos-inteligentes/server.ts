...
async function startServer() {
  ...
  function loadStorage() {
    try {
      ...
      if ((dataset.tables as any).access_requests?.data) accessRequests = (dataset.tables as any).access_requests.data as any;
    } catch (e) {
  ...
  function saveStorage() {
    try {
      ...
      if (!(dataset.tables as any).access_requests) {
        (dataset.tables as any).access_requests = { name: "access_requests", description: "Portal access requests", schema: [], data: [] };
      }
      (dataset.tables as any).access_requests.data = accessRequests as any;
  ...
  const accessRequestAttempts = new Map<string, number[]>();
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    ...
    if (user.status === 'PENDING') {
      ...
      organizationMemberships
        ...
        .forEach(membership => { membership.status = 'ACTIVE'; });
    }
  ...
  const clientToolCatalog = [
    ...
    },
    {
      ...
      url: process.env.PAINEL_OBRIGACOES_URL || 'https://obrigacoes.e3isolucoes.com.br/'
    }
  ];
  ...
  app.post('/api/access-requests', async (req, res) => {
    const requestKey = String(req.ip || req.socket.remoteAddress || 'unknown');
    const now = Date.now();
    const recentAttempts = (accessRequestAttempts.get(requestKey) || []).filter(timestamp => now - timestamp < 60 * 60_000);
    if (recentAttempts.length >= 5) {
      return sendError(req, res, 429, 'RATE_LIMITED', 'Limite de solicitações atingido. Tente novamente mais tarde.');
    }
    accessRequestAttempts.set(requestKey, [...recentAttempts, now]);

    const name = String(req.body?.name || '').trim().replace(/\s+/g, ' ');
    const corporateEmail = String(req.body?.corporateEmail || '').trim().toLowerCase();
    ...
    const requestedToolIds: string[] = Array.from(new Set<string>(Array.isArray(req.body?.requestedToolIds) ? req.body.requestedToolIds.map((value: unknown) => String(value)) : []));
    const validToolIds = new Set(clientToolCatalog.map(tool => tool.id));

    if (name.length < 3 || companyName.length < 3 || jobTitle.length < 2 || phone.replace(/\D/g, '').length < 10) {
      return sendError(req, res, 400, 'INVALID_ACCESS_REQUEST', 'Preencha nome, empresa, cargo e telefone válidos.');
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(corporateEmail) || /@(gmail|hotmail|outlook|yahoo|icloud)\./i.test(corporateEmail)) {
      return sendError(req, res, 400, 'CORPORATE_EMAIL_REQUIRED', 'Informe um e-mail corporativo válido.');
    }
    if (document.length !== 14 || requestedToolIds.length === 0 || requestedToolIds.some(id => !validToolIds.has(id))) {
      return sendError(req, res, 400, 'INVALID_ACCESS_SCOPE', 'Informe um CNPJ válido e ao menos uma ferramenta disponível.');
    }
    if (!privacyAccepted) {
      return sendError(req, res, 400, 'PRIVACY_CONSENT_REQUIRED', 'Confirme a autorização para análise dos dados informados.');
    }

    const duplicate = accessRequests.find(item => item.corporateEmail === corporateEmail && item.document === document && item.status === 'PENDING');
    let requestRecord = duplicate;
    let notificationStatus: 'SENT' | 'FAILED' | 'NOT_CONFIGURED' | 'ALREADY_PENDING' = duplicate ? 'ALREADY_PENDING' : 'NOT_CONFIGURED';
    if (!duplicate) {
      const request: AccessRequest = {
        id: `access-${crypto.randomUUID()}`,
        name, corporateEmail, phone, jobTitle, companyName, document,
        requestedToolIds, reason, status: 'PENDING', createdAt: new Date().toISOString()
      };
      requestRecord = request;
      accessRequests.unshift(request);
      saveStorage();
      recordAuditEvent(req, { organizationId: 'public', actorUserId: 'anonymous', targetType: 'ACCESS_REQUEST', targetId: request.id, action: 'ACCESS_REQUEST_CREATED', result: 'SUCCESS', metadata: { requestedToolIds, companyName } });

      if (process.env.RESEND_API_KEY) {
        const from = process.env.ACCESS_REQUEST_FROM_EMAIL || process.env.PASSWORD_RESET_FROM_EMAIL || 'Portal E3I <onboarding@resend.dev>';
        const toolNames = clientToolCatalog.filter(tool => requestedToolIds.includes(tool.id)).map(tool => tool.name).join(', ');
        try {
          const notificationResponse = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              from,
              to: ['admin@e3isolucoes.com.br'],
              reply_to: corporateEmail,
              subject: `Nova solicitação de acesso — ${companyName}`,
              html: `<h2>Nova solicitação de acesso</h2><p><b>Solicitante:</b> ${escapeHtml(name)} — ${escapeHtml(jobTitle)}</p><p><b>Empresa:</b> ${escapeHtml(companyName)} · CNPJ ${escapeHtml(document)}</p><p><b>E-mail:</b> ${escapeHtml(corporateEmail)}<br><b>Telefone:</b> ${escapeHtml(phone)}</p><p><b>Ferramentas:</b> ${escapeHtml(toolNames)}</p><p><b>Justificativa:</b> ${escapeHtml(reason || 'Não informada')}</p><p>Acesse o Portal E3I para analisar a solicitação.</p>`
            })
          });
          if (!notificationResponse.ok) {
            throw new Error(`RESEND_HTTP_${notificationResponse.status}`);
          }
          notificationStatus = 'SENT';
          recordAuditEvent(req, { organizationId: 'public', actorUserId: 'system', targetType: 'ACCESS_REQUEST', targetId: request.id, action: 'ACCESS_REQUEST_NOTIFICATION_SENT', result: 'SUCCESS' });
        } catch (error) {
          notificationStatus = 'FAILED';
          recordAuditEvent(req, { organizationId: 'public', actorUserId: 'system', targetType: 'ACCESS_REQUEST', targetId: request.id, action: 'ACCESS_REQUEST_NOTIFICATION_FAILED', result: 'WARNING', metadata: { provider: 'resend' } });
          console.error('Access request notification failed:', error instanceof Error ? error.message : 'UNKNOWN');
        }
      }
    }
    ...
    res.status(202).json({
      ...
      notificationStatus,
      message: duplicate
        ? 'Já existe uma solicitação pendente para este e-mail e CNPJ. A equipe E3I seguirá com a análise.'
        : 'Solicitação registrada. A equipe E3I analisará os dados antes de conceder o acesso.'
    });
  });
  ...
  app.get('/api/admin/access-requests', (req, res) => {
    const authResult = validateSession(req, res);
    if (!authResult) return;
    if (authResult.tenantContext.systemRole !== 'E3I_ADMIN') return res.status(403).json({ error: 'Acesso restrito à administração E3I.' });
    res.json({ requests: accessRequests, tools: clientToolCatalog.map(({ id, name }) => ({ id, name })) });
  });

  async function sendAccessActivationEmail(request: AccessRequest, approvedUser: (typeof users)[number], organization: (typeof tenants)[number]) {
    if (approvedUser.status === 'ACTIVE') return { status: 'NOT_REQUIRED' as const };
    if (!process.env.RESEND_API_KEY || !process.env.PASSWORD_RESET_FROM_EMAIL) {
      return { status: 'NOT_CONFIGURED' as const };
    }

    const resetToken = crypto.randomBytes(32).toString('hex');
    const ttlMinutes = Math.max(15, Math.min(1440, Number(process.env.ACCESS_ACTIVATION_TOKEN_TTL_MINUTES) || 120));
    passwordResetTokens[resetToken] = { userId: approvedUser.id, expiresAt: Date.now() + ttlMinutes * 60_000 };
    const appUrl = (process.env.APP_URL || 'https://portal.e3isolucoes.com.br').replace(/\/$/, '');
    const activationLink = `${appUrl}/?resetToken=${encodeURIComponent(resetToken)}`;
    try {
      const mailResponse = await fetch('https://api.resend.com/emails', {
        method: 'POST',
        headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          subject: 'Acesso aprovado — Portal E3I Soluções',
          html: `<p>Olá ${escapeHtml(approvedUser.name)},</p><p>Seu acesso ao Portal E3I foi aprovado para a organização <b>${escapeHtml(organization.name)}</b>.</p><p><a href="${activationLink}">Definir minha senha e ativar o acesso</a></p><p>Este link expira em ${ttlMinutes} minutos.</p>`
        })
      });
      if (!mailResponse.ok) throw new Error(`EMAIL_HTTP_${mailResponse.status}`);
      request.activationLastSentAt = new Date().toISOString();
      return { status: 'SENT' as const };
    } catch (error) {
      delete passwordResetTokens[resetToken];
      console.error('Access activation email delivery failed:', error instanceof Error ? error.message : 'UNKNOWN');
      return { status: 'FAILED' as const };
    }
  }

  app.patch('/api/admin/access-requests/:requestId', async (req, res) => {
    const authResult = validateSession(req, res);
    if (!authResult) return;
    if (authResult.tenantContext.systemRole !== 'E3I_ADMIN') return res.status(403).json({ error: 'Acesso restrito à administração E3I.' });
    const request = accessRequests.find(item => item.id === req.params.requestId);
    if (!request) return res.status(404).json({ error: 'Solicitação não encontrada.' });
    if (request.status !== 'PENDING') return res.status(409).json({ error: 'Esta solicitação já foi analisada.' });
    const decision = String(req.body?.decision || '').toUpperCase();
    const createOrganization = req.body?.createOrganization === true;
    if (!['APPROVED', 'REJECTED'].includes(decision)) return res.status(400).json({ error: 'Decisão inválida.' });

    let organizationCreated = false;
    let activationNotificationStatus: 'NOT_REQUIRED' | 'SENT' | 'FAILED' | 'NOT_CONFIGURED' = 'NOT_REQUIRED';
    if (decision === 'APPROVED') {
      let organization = tenants.find(item => item.document.replace(/\D/g, '') === request.document);
      if (!organization && !createOrganization) {
        return res.status(409).json({
          code: 'ORGANIZATION_REQUIRED',
          error: 'A empresa ainda não está cadastrada. Confirme o cadastro e a aprovação para continuar.'
        });
      }
      if (!organization) {
        organization = {
        ...
        tenants.push(organization);
        organizationCreated = true;
        recordAuditEvent(req, { organizationId: organization.id, actorUserId: authResult.user.id, targetType: 'ORGANIZATION', targetId: organization.id, action: 'ORGANIZATION_CREATED_FROM_ACCESS_REQUEST', result: 'SUCCESS', metadata: { document: request.document, accessRequestId: request.id } });
      }
      if (organization.status !== 'ACTIVE') {
        return res.status(409).json({ code: 'ORGANIZATION_INACTIVE', error: 'A organização está inativa. Reative-a antes de conceder acessos.' });
      }

      const existingUser = users.find(item => item.email.toLowerCase() === request.corporateEmail);
      if (existingUser && existingUser.tenantId !== organization.id) {
        return res.status(409).json({ code: 'EMAIL_ALREADY_LINKED', error: 'Este e-mail já está vinculado a outra organização. Revise o cadastro antes de aprovar.' });
      }
      ...
      if (!approvedUser) {
        approvedUser = {
          ...
          status: 'PENDING',
          lastLogin: 'Aguardando definição de senha',
          passwordHash: await passwordHasher.hash(crypto.randomBytes(32).toString('hex')),
          mustChangePassword: true,
        };
        users.push(approvedUser);
        organization.usersCount += 1;
      }
      const membership = ensureOrganizationMembership(approvedUser, organization.id);
      if (approvedUser.status === 'ACTIVE') membership.status = 'ACTIVE';

      organization.toolAccess = Array.from(new Set([...(organization.toolAccess || []), ...request.requestedToolIds]));
      request.organizationId = organization.id;

      const activationResult = await sendAccessActivationEmail(request, approvedUser, organization);
      activationNotificationStatus = activationResult.status;
      request.activationNotificationStatus = activationNotificationStatus;
    }
    request.status = decision as 'APPROVED' | 'REJECTED';
    request.reviewedAt = new Date().toISOString();
    request.reviewedBy = authResult.user.id;
    saveStorage();
    recordAuditEvent(req, { organizationId: request.organizationId || authResult.tenant.id, actorUserId: authResult.user.id, targetType: 'ACCESS_REQUEST', targetId: request.id, action: `ACCESS_REQUEST_${decision}`, result: 'SUCCESS', metadata: { requestedToolIds: request.requestedToolIds } });
    res.json({ success: true, request, organizationCreated, activationNotificationStatus });
  });

  app.post('/api/admin/access-requests/:requestId/resend-activation', async (req, res) => {
    const authResult = validateSession(req, res);
    if (!authResult) return;
    if (authResult.tenantContext.systemRole !== 'E3I_ADMIN') return res.status(403).json({ error: 'Acesso restrito à administração E3I.' });
    const request = accessRequests.find(item => item.id === req.params.requestId);
    if (!request) return res.status(404).json({ error: 'Solicitação não encontrada.' });
    if (request.status !== 'APPROVED' || !request.organizationId) return res.status(409).json({ error: 'A solicitação precisa estar aprovada e vinculada a uma organização.' });
    if (request.activationLastSentAt && Date.now() - new Date(request.activationLastSentAt).getTime() < 60_000) {
      return res.status(429).json({ error: 'Aguarde um minuto antes de reenviar o link de ativação.' });
    }
    const organization = tenants.find(item => item.id === request.organizationId);
    const approvedUser = users.find(item => item.email.toLowerCase() === request.corporateEmail.toLowerCase() && item.tenantId === request.organizationId);
    if (!organization || !approvedUser) return res.status(404).json({ error: 'Usuário ou organização vinculada não foi encontrado.' });
    if (approvedUser.status === 'ACTIVE') return res.status(409).json({ code: 'ACCOUNT_ALREADY_ACTIVE', error: 'A conta já está ativa e pode acessar o portal.' });

    const activationResult = await sendAccessActivationEmail(request, approvedUser, organization);
    request.activationNotificationStatus = activationResult.status;
    saveStorage();
    recordAuditEvent(req, {
      organizationId: organization.id,
      actorUserId: authResult.user.id,
      ...
      targetId: request.id,
      action: 'ACCESS_ACTIVATION_RESEND',
      result: activationResult.status === 'SENT' ? 'SUCCESS' : 'WARNING',
      metadata: { deliveryStatus: activationResult.status }
    });
    if (activationResult.status === 'NOT_CONFIGURED') return res.status(503).json({ request, code: 'ACTIVATION_EMAIL_NOT_CONFIGURED', error: 'O serviço de e-mail de ativação não está configurado.' });
    if (activationResult.status === 'FAILED') return res.status(502).json({ request, code: 'ACTIVATION_EMAIL_FAILED', error: 'O provedor recusou o envio. Verifique o domínio remetente e tente novamente.' });
    res.json({ success: true, request, activationNotificationStatus: activationResult.status });
  });
  ...
  async function createObligationsLaunchUrl(user: (typeof users)[number], tenant: (typeof tenants)[number], baseUrl: string) {
    const supabaseUrl = (process.env.PAINEL_SUPABASE_URL || '').replace(/\/$/, '');
    const serviceRoleKey = process.env.PAINEL_SUPABASE_SERVICE_ROLE_KEY || '';
    if (!supabaseUrl || !serviceRoleKey) throw new Error('OBLIGATIONS_SSO_NOT_CONFIGURED');
    const adminHeaders = { Authorization: `Bearer ${serviceRoleKey}`, apikey: serviceRoleKey, 'Content-Type': 'application/json' };
    const document = tenant.document.replace(/\D/g, '');

    const workspaceResponse = await fetch(`${supabaseUrl}/rest/v1/workspaces?document=eq.${encodeURIComponent(document)}&select=id,access_status&limit=1`, { headers: adminHeaders });
    if (!workspaceResponse.ok) throw new Error(`OBLIGATIONS_WORKSPACE_LOOKUP_${workspaceResponse.status}`);
    let workspace = (await workspaceResponse.json() as Array<{ id: string; access_status: string }>)[0];
    if (!workspace) {
      const createWorkspaceResponse = await fetch(`${supabaseUrl}/rest/v1/workspaces`, {
        method: 'POST', headers: { ...adminHeaders, Prefer: 'return=representation' },
        body: JSON.stringify({ name: tenant.name, document, access_status: 'full' })
      });
      if (!createWorkspaceResponse.ok) throw new Error(`OBLIGATIONS_WORKSPACE_CREATE_${createWorkspaceResponse.status}`);
      workspace = (await createWorkspaceResponse.json() as Array<{ id: string; access_status: string }>)[0];
    } else if (workspace.access_status === 'suspended') {
      throw new Error('OBLIGATIONS_WORKSPACE_SUSPENDED');
    }

    const profileLookupResponse = await fetch(`${supabaseUrl}/rest/v1/profiles?email=eq.${encodeURIComponent(user.email)}&select=id&limit=1`, { headers: adminHeaders });
    if (!profileLookupResponse.ok) throw new Error(`OBLIGATIONS_USER_LOOKUP_${profileLookupResponse.status}`);
    let panelUser = (await profileLookupResponse.json() as Array<{ id: string }>)[0];
    if (!panelUser) {
      const createUserResponse = await fetch(`${supabaseUrl}/auth/v1/admin/users`, {
        method: 'POST', headers: adminHeaders,
        body: JSON.stringify({ email: user.email, email_confirm: true, user_metadata: { display_name: user.name } })
      });
      if (!createUserResponse.ok) throw new Error(`OBLIGATIONS_USER_CREATE_${createUserResponse.status}`);
      panelUser = await createUserResponse.json();
    }
    ...
    const profileResponse = await fetch(`${supabaseUrl}/rest/v1/profiles?on_conflict=id`, {
      ...
      body: JSON.stringify({ id: panelUser.id, email: user.email, workspace_id: workspace.id, display_name: user.name, role: 'membro', active: true })
    });
    if (!profileResponse.ok) throw new Error(`OBLIGATIONS_PROFILE_LINK_${profileResponse.status}`);

    const canonicalUrl = baseUrl.replace(/\/$/, '');
    const linkResponse = await fetch(`${supabaseUrl}/auth/v1/admin/generate_link`, {
      method: 'POST', headers: adminHeaders,
      body: JSON.stringify({ type: 'magiclink', email: user.email, options: { redirectTo: canonicalUrl } })
    });
    if (!linkResponse.ok) throw new Error(`OBLIGATIONS_LINK_CREATE_${linkResponse.status}`);
    const linkPayload: any = await linkResponse.json();
    const tokenHash = linkPayload?.properties?.hashed_token;
    if (!tokenHash) throw new Error('OBLIGATIONS_LINK_INVALID');
    const separator = canonicalUrl.includes('?') ? '&' : '?';
    return `${canonicalUrl}${separator}portal_sso_token=${encodeURIComponent(tokenHash)}&portal_sso_type=magiclink`;
  }

  app.post('/api/client-tools/:toolId/launch', async (req, res) => {
    const authResult = validateSession(req, res);
    if (!authResult) return;
    const { user, tenant } = authResult;
    ...
    if (tool.id === 'painel-obrigacoes') {
      try {
        launchUrl = await createObligationsLaunchUrl(user, tenant, tool.url);
      } catch (error) {
        const reason = error instanceof Error ? error.message : 'OBLIGATIONS_SSO_FAILED';
        recordAuditEvent(req, { organizationId: tenant.id, actorUserId: user.id, action: 'CLIENT_TOOL_SSO_FAILED', result: 'FAILURE', metadata: { toolId: tool.id, reason } });
        if (reason === 'OBLIGATIONS_SSO_NOT_CONFIGURED') return sendError(req, res, 503, reason, 'A integração de acesso único do Painel de Obrigações ainda não está configurada.');
        if (reason === 'OBLIGATIONS_WORKSPACE_SUSPENDED') return sendError(req, res, 403, reason, 'O ambiente do Painel de Obrigações está suspenso para esta organização.');
        return sendError(req, res, 502, 'OBLIGATIONS_SSO_FAILED', 'Não foi possível preparar o acesso automático ao Painel de Obrigações.');
      }
    }
    recordAuditEvent(req, {
      organizationId: tenant.id,
