...
export const Navbar: React.FC = () => {
  ...
  const processosNavItems = [
    { id: 'dashboard', label: 'Visão Geral', icon: <LayoutDashboard className="w-4 h-4" />, show: canUseProcessos },
  ...
  const processosViews = new Set(['dashboard', 'discovery', 'discoveryReview', 'businessContext', 'strategyCanvas', 'organizationMap', 'systems']);
  const isProcessosWorkspace = processosViews.has(currentView);
  ...
  const showSidebar = sidebarViews.has(currentView);

  const adminNavItems = [
    { id: 'accessRequests', label: 'Solicitações de Acesso', icon: <Inbox className="w-4 h-4" />, show: user?.role === 'E3I_ADMIN' },
        ...
        <div className="flex flex-col h-full justify-between p-4 overflow-y-auto">
          <div className="space-y-6">
            ...
            <div className="space-y-1.5">
              {!isCollapsed && (
                <p className="px-3 text-[10px] font-bold text-text-secondary uppercase tracking-wider">Portal</p>
              )}
            ...
            {isProcessosWorkspace && canUseProcessos && (
              <div className="space-y-1.5 border-t border-border-subtle pt-4">
                {!isCollapsed && (
                  <p className="px-3 text-[10px] font-bold uppercase tracking-wider text-gold">E3I Processos Inteligentes</p>
                )}
                {processosNavItems.filter(item => item.show).map((item) => {
                  const isActive = currentView === item.id || (item.id === 'discovery' && currentView === 'discoveryReview');
                  return (
                    <button key={item.id} onClick={() => setCurrentView(item.id as any)} title={isCollapsed ? item.label : undefined} className={`group relative flex w-full items-center space-x-3 rounded-xl px-3.5 py-3 text-sm font-medium transition-all ${isActive ? 'bg-accent/15 font-semibold text-accent shadow-sm' : 'text-text-secondary hover:bg-surface-raised hover:text-text-primary'}`}>
                      {isActive && <span className="absolute bottom-2 left-0 top-2 w-1 rounded-r-full bg-accent" />}
                      <span className="shrink-0">{item.icon}</span>
                      {!isCollapsed && <span className="truncate">{item.label}</span>}
                    </button>
                  );
          ...
          {!isCollapsed && (
            <div className="p-3 rounded-2xl bg-surface-raised border border-border-subtle text-xs space-y-1">
              <p className="font-bold text-text-primary">E³I Portal Empresarial</p>
              <p className="text-[11px] text-text-muted">Acesso seguro às soluções</p>
            </div>
          )}
        </div>
      </aside>}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <p className="px-3 text-[10px] font-bold text-text-secondary uppercase tracking-wider mb-2">Portal</p>
                    {portalNavItems.map(item => {
                      const isActive = currentView === item.id;
                      return (
                        <button
                  ...
                  {isProcessosWorkspace && canUseProcessos && (
                    <div className="space-y-1 border-t border-border-subtle pt-4">
                      <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-wider text-gold">E3I Processos Inteligentes</p>
                      {processosNavItems.filter(item => item.show).map(item => {
                        const isActive = currentView === item.id || (item.id === 'discovery' && currentView === 'discoveryReview');
                        return (
                          <button key={item.id} onClick={() => { setCurrentView(item.id as any); setMobileDrawerOpen(false); }} className={`flex w-full items-center space-x-3 rounded-xl px-4 py-3 text-sm font-medium transition-all ${isActive ? 'bg-accent/15 font-semibold text-accent' : 'text-text-secondary hover:bg-surface-raised hover:text-text-primary'}`}>
